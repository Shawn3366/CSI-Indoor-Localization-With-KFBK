...
def compute_threshold(coeffs):
    ...
    return 0.8 * sigma * np.sqrt(2 * np.log(len(coeffs[-1])))  # 仅用细节系数长度计算阈值
...
def getOriginalCSI():
    ...
    yLabel = getYlabel()
    count = 0
    originalCSI = np.zeros((176, 135000), dtype=float)
    label = np.empty((0, 2), dtype=int)

    for i in range(16):
        for j in range(11):
            # filePath = r"/Users/zhuxiaoqiang/Downloads/我的定位数据集/55SwapData/coordinate" + xLabel[i] + yLabel[j] + ".mat"
            filePath = "D:/system_default/desk_top/he/BLS_KAN_Location/BLS_KAN_Location/55SwapData/coordinate" + xLabel[i] + yLabel[
                j] + ".mat"
...
def generatePilot():
    ...
    for i in range(originalData.shape[0]):
        coeffs = pywt.wavedec(originalData[i], 'sym5', level=3)
        sigma = np.median(np.abs(coeffs[-1])) / 0.6745
        uthresh = sigma * np.sqrt(2*np.log(len(originalData[i])))
        coeffs = [pywt.threshold(c, value=uthresh, mode='soft') for c in coeffs]
        originalData[i] = pywt.waverec(coeffs, 'sym5')

    # 添加PCA降维
    pca = PCA(n_components=0.95)
    originalData = pca.fit_transform(originalData)

    rng = np.random.RandomState(20)
    randomLabel = rng.randint(1, 176, size=18)
...
def filterProcess(mulGauProPrediction, n_iter):
    from pykalman import KalmanFilter
    from scipy import signal
    bufferCSI = np.zeros((len(mulGauProPrediction), len(mulGauProPrediction[0])), dtype=float)
...
def findPossiblePath(stateFile):
    possiblePath = []
    stateLabel = []
    state = loadmat(r"D:/system_default/desk_top/he/BLS_KAN_Location/BLS_KAN_Location/Fifth code Meet/"+ stateFile)
...
def OptimalPath(rewardFile):
    ...
    reward = loadmat(r"D:/system_default/desk_top/he/BLS_KAN_Location/BLS_KAN_Location/Fifth code Meet/" + rewardFile)
...
if __name__ == '__main__':
    ...
    # secondpilotCSI_cl = np.abs(secondpilotCSI)
    secondpilotCSI_cl = secondpilotCSI
    # 假设  是一个 numpy 的 ndarray，你需要将其转换为 pandas DataFrame
    ...
    model = DeepBLS(max_iter=50,
                 ...
                 s=0.8,
                 C=2 ** -30)
    # model.fit(corresponding_labels,secondpilotCSI_cl_processed32)
    ...
    GAN_data = sort_data.reshape((176, 3 * 30 * 1500))
    # 在生成数据后添加
